classdef SolidShellLayered_v5 < handle
    ...
    %layer (out of plane components). Inplane stress components are
    ...
    methods
        function obj = SolidShellLayered_v5(ngpx, ngpy, ngpz, ex, ey, ez, stressInterp, Mhat, elprop)
        ...
        function [R, J] = computeRandJ(obj,a, eq, ftrac, elprop)
            ...
            LLi= zeros(obj.nStressDofs, obj.nStrainDofs);%nStrainDofs
...
%                             [1,1,1,1]*elprop.int_coordsG(ilay+1)];
                ...
                KK2(:,m) = KK2(:,m) + Ke2;
%                 LLi(m,:) = LLi(m,:) + Li;
%                 LLi(m,(1:obj.nLamStrainDofs) + (obj.nLamStrainDofs*(ilay-1))) = LLi(m,(1:obj.nLamStrainDofs) + (obj.nLamStrainDofs*(ilay-1))) + Li;
                
                LCAsum(m,:) = LCAsum(m,:) + Li*inv(Ci)*Ai;
            end
